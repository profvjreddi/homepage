...
Evaluation: Orchestration benchmarks. Capability is improving, but no benchmark tests what matters most for production orchestration: coordination under failure. We need benchmarks that measure whether rollback preserves consistency, whether agents recover gracefully from partial execution, and whether orchestrated systems satisfy their stated constraints after both successful and failed runs.

These domains are interdependent. This viewpoint is simply a call to action, not a report of completed research. Progress on any of these fronts advances the field; solving them transforms how we build systems.
...
If English is a programming language, then everyone is a programmer, and the database that gets deleted is not the engineer's experiment but the small business's livelihood.

The question is no longer whether natural language will be used for orchestration. It already is. The question is whether we deliberately build the formal foundations to make it safe, or whether we accept a future where our infrastructure is managed by vibe coding until it inevitably fails.

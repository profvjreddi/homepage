...
The failure modes are predictable to anyone who has built these systems:

- A model that performed well in testing degrades silently in production because the input distribution shifted, and no one built monitoring to detect it.
- A system that worked in the lab fails at the edge because latency requirements were never communicated to the team selecting model architectures.
- A team retrained a model without versioning the training data and cannot reproduce last week's results.
- A deployment pipeline continues to run until the model size exceeds the available memory, a constraint that has not been verified.
...
Data engineering for ML encompasses not only the mechanics of data pipelines but also the deeper challenges of data quality, provenance, versioning, and the feedback loops that arise when model outputs influence future training data. Sambasivan and colleagues documented how conventional AI practices systematically undervalue data quality, leading to "data cascades" that compound negative downstream effects \[6\]. Their finding that "everyone wants to do the model work, not the data work" points to a cultural problem that disciplinary recognition could help address.<sidenote>This cultural bias is pervasive. Andrew Ng observed that if AI is the new electricity, we're training everyone to use the appliances while almost no one learns how to build the power plants.</sidenote> Model development extends beyond training to include selecting appropriate architectures for deployment constraints, designing evaluation protocols that predict production performance, and developing baselines that enable meaningful comparison. Amershi and colleagues found that AI teams face challenges distinct from traditional software: discovering and managing data is fundamentally more complex, model customization requires different skills, and AI components resist modular decomposition \[7\].

### Systems, Operations, and Reliability

Once models move from experimentation into real systems, new constraints emerge. Systems integration addresses how ML components interact with the broader systems in which they are embedded: API design, latency management, graceful degradation, and maintaining consistency across model versions. Deployment and operations cover the infrastructure for serving models, the monitoring systems that detect degradation, the processes for safe rollout and rollback, and the automation that enables continuous improvement. Safety and reliability encompass not only robustness to adversarial inputs but also ensuring that AI systems behave acceptably across conditions not represented in training data. These concerns are deeply intertwined: a system that cannot be monitored cannot be made reliable, and a system that cannot be safely rolled back cannot be safely deployed.
...
These four areas are interconnected. Decisions in one area constrain options in others. A central competency of the AI engineer is the systematic navigation of these trade-offs rather than their discovery through trial and error.

## Why Now

Four developments make disciplinary recognition urgent.

First, **the gap between research and production has become economically significant**. Organizations are investing heavily in AI capabilities but struggling to realize value. The problem is not a shortage of ML talent. There is a shortage of people who can bridge the gap between what ML can do in principle and what systems can do in practice. This is a workforce problem that requires an educational response.<sidenote>Students sense this shift: <a href="https://cra.org/cerp/pulse-survey/">CS enrollment is down at 62% of computing programs</a>, while engineering overall is up 7.5%. The CRA notes students are gravitating toward majors that feel "more physical and less susceptible to the impact of AI."</sidenote> Current training, as multiple studies have documented, leaves graduates prepared to train models on clean datasets but unprepared for the realities of production data, system integration, and operational maintenance \[6, 7\].

Second, **AI systems are increasingly consequential**. They influence hiring decisions, medical diagnoses, financial outcomes, and physical safety. The ad hoc approaches that sufficed when AI was experimental are inadequate for systems that affect people's lives. We need professional standards, and professional standards require a professional identity.

Third, **the economics of scale demand an engineering discipline**. Training frontier models now costs tens of millions of dollars and consumes staggering amounts of energy; by some estimates, training a single large model can require over 50 gigawatt-hours of electricity, comparable to the annual consumption of thousands of households.<sidenote>The infrastructure buildout is staggering: <a href="https://www.wsj.com/tech/ai/openai-stargate-ai-infrastructure-project-b4a5e19c">OpenAI has committed to $1.15 trillion</a> in infrastructure over the next decade. Microsoft's <a href="https://news.microsoft.com/source/features/sustainability/inside-fairwater-worlds-most-powerful-ai-datacenter/">Fairwater datacenter</a> covers 315 acres with 26.5 million pounds of steel. Analysts predict global AI infrastructure investment will <a href="https://www.goldmansachs.com/insights/articles/ai-investment-forecast-to-approach-200-billion-globally-by-2025">exceed $1 trillion by 2030</a>.</sidenote> Operating these models at scale increases costs by many orders of magnitude. The brute-force approach of increasing data, parameters, and compute is encountering physical and economic limits.

Fourth, **AI is moving to the edge**. Models are being deployed on mobile devices, embedded systems, vehicles, and industrial equipment, environments where the assumptions of cloud-based ML break down.<sidenote>The <a href="https://www.marketsandmarkets.com/Market-Reports/tiny-ml-market-254788457.html">TinyML market is growing at 34% annually</a>, driven by demand for on-device inference in phones, wearables, medical devices, and industrial equipment.</sidenote> These systems must operate reliably with limited connectivity, constrained compute, and no opportunity for rapid rollback. The hardware constraints are fixed for product lifetimes measured in years. The update mechanisms are slow, risky, and often require physical access. Failures may have physical consequences.<sidenote>NVIDIA calls this "Physical AI" — Jensen Huang recently declared that "<a href="https://nvidianews.nvidia.com/news/nvidia-announces-latest-physical-ai-models">the ChatGPT moment for robotics is here</a>." Robotics is now the fastest-growing category on Hugging Face.</sidenote> The engineering challenges are severe, and they are not well addressed by either cloud-focused MLOps practices or traditional embedded systems engineering. Edge AI demands a synthesis that neither field currently provides. 
...
The patterns of failure and the gaps in practitioner training recur across domains and organizations.<sidenote>These observations of mine emerge from building ML systems across scales—from datacenter-scale <a href="https://mlcommons.org/benchmarks/training/">MLPerf benchmarks</a> to microcontroller-scale <a href="https://www.edx.org/professional-certificate/harvardx-tiny-machine-learning">TinyML deployments</a>—and from efforts to codify this knowledge through open curricula: <a href="https://mlsysbook.ai">mlsysbook.ai</a> and <a href="https://tinytorch.ai">tinytorch.ai</a>.</sidenote> In 1968 and 1977, the computing community recognized that building reliable software and hardware systems required disciplines distinct from computer science. Software engineering and computer engineering emerged to fill those gaps. **If AI really is the new electricity, we need more than researchers discovering new phenomena—we need engineers who can build the grid.**

## References

\[1\] M. Shaw. ["Prospects for an Engineering Discipline of Software."](https://sei.cmu.edu/library/prospects-for-an-engineering-discipline-of-software) Carnegie Mellon University, Software Engineering Institute, Technical Report CMU/SEI-90-TR-20, 1990.
